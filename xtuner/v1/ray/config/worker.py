...
class RolloutConfig(BaseModel):
    ...
    **Examples:**
    ...
    servers_per_engine: Annotated[
        int,
        Parameter(
            group=infer_group,
            help="Number of servers per inference engine for the rollout worker.",
        ),
    ] = 1
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        assert self.servers_per_engine == 1, "servers_per_engine is not configurable by users for now."
        if rollout_backend == "lmdeploy" and self.expert_parallel_size > 1:
            ...
            self.servers_per_engine = self.expert_parallel_size
